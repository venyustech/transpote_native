...
export default class SideMenu extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      sideMenuList:
        [
          {
            key: 1, name: languageJSON.book_your_ride_menu,
            navigationName: 'Map',
            icon: 'home',
            type: 'font-awesome',
            child: 'firstChild'
          ...
          {
            key: 4,
            name: languageJSON.payment_method_title,
            navigationName: 'PaymentMethodScreen',
          ...
          { key: 6, name: "Ajuda", navigationName: 'Help', icon: 'question', type: 'font-awesome', child: 'sixthChild' },
  ...
  componentDidMount() {
    ...
    this.userRoot = firebase.database().ref('users/' + curuser);
    this.userRoot.on('value', userData => {
      if (userData.val()) {
  ...
  tripSatusCheck() {
    ...
    this.setState({ currentUser: curuser }, () => {
      this.userData = firebase.database().ref('users/' + this.state.currentUser.uid);
      this.userData.on('value', userData => {
        if (userData.val()) {
          var data = userData.val()

          // Pagamento concluído via AppCredit redirecionar
          if (data.redirectInit && data.redirectInit == true) {
            console.log('Apagar referencia redirectInit');
            firebase.database().ref(`users/${this.state.currentUser.uid}`).update({
              ...
              console.log('Falha ao realizar update', err);
            });
            return;
          } else if (data['my-booking']) {

...
export default class BookedCabScreen extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      driverInfoModalVisible: false,
      messagePleaseWait: 'Estamos buscando prestadores...',
      indexMessageWait: 1,
      coords: [],
      radio_props: [],
      value: 0,
  ...
  componentDidMount() {
    ...
    const bookingResponse = firebase
      .database()
      .ref(
        'users/' + curuser.uid + '/my-booking/' + this.getParamData.bokkingId
      )
    bookingResponse.on('value', currUserBookings => {
      if (currUserBookings.val()) {
        ...
        if (typeof currUserBookings.val().vehicle_color === 'string') {
          switch (currUserBookings.val().vehicle_color.toLowerCase()) {
            case 'amarelo':
              this.setState({
                vehicleColor: require('../../assets/images/cars/amarelo.png')
              })
              break
            case 'azul':
              this.setState({
                vehicleColor: require('../../assets/images/cars/azul.png')
              })
              break
            case 'branco':
              this.setState({
                vehicleColor: require('../../assets/images/cars/branco.png')
              })
              break
            case 'dourado':
              this.setState({
                vehicleColor: require('../../assets/images/cars/dourado.png')
              })
              break
            case 'prata':
              this.setState({
                vehicleColor: require('../../assets/images/cars/prata.png')
              })
              break
            case 'verde':
              this.setState({
                vehicleColor: require('../../assets/images/cars/verde.png')
              })
              break
            case 'vermelho':
              this.setState({
                vehicleColor: require('../../assets/images/cars/vermelho.png')
              })
              break
            default:
              this.setState({
        ...
        this.setState(
          {
            ...
            vehicleColorName: currUserBookings.val().vehicle_color,
        ...
        if (currUserBookings.val().status == 'ACCEPTED') {
          ...
          this.props.navigation.navigate('trackRide', {
            data: currUserBookings.val(),
            bId: this.getParamData.bokkingId
          })
        }
      }
    })
  ...
  async getDirections(startLoc, destinationLoc) {
    try {
      const resp = await fetch(
        `https://maps.googleapis.com/maps/api/directions/json?origin=${startLoc}&destination=${destinationLoc}&key=${googleMapKey}`
      )
  ...
  listMessageInfo() {
    const listInfo = [
      'Estamos buscando prestadores...',
      'Caso demore considere aumentar sua oferta...',
      'Evite contato, utilize sua carteira virtual...',
      'Esteja pronto quando seu motorista chegar...',
      'Caso demore experimente trocar a forma de pagamento...'
    ]

    this.messageTime = setInterval(() => {
      let index = this.state.indexMessageWait
      if (index > 4) {
        index = 0
      }

      this.setState({
        messagePleaseWait: listInfo[index],
        indexMessageWait: index + 1
      })
    }, 7000)
  }
      ...
      setTimeout(() => {
        ...
        firebase
          ...
          .then(() => {
            ...
            const requestedDriver = firebase
              .database()
              .ref(
                'bookings/' + this.state.currentBookingId + '/requestedDriver'
              )
            requestedDriver.once('value', drivers => {
              if (drivers.val()) {
                const requetedDrivers = drivers.val()
                let count = 0
            ...
            firebase
              .database()
              .ref('bookings/' + this.state.currentBookingId + '/')
              .update({
                status: 'CANCELLED',
                reason: this.state.radio_props[this.state.value].label
              })
              .then(() => {
                ...
                firebase
                  ...
                  .ref(
                    '/users/' +
                      this.state.driverUID +
                      '/my_bookings/' +
                      this.state.currentBookingId +
                      '/'
                  )
                  .on('value', curbookingData => {
                    if (curbookingData.val()) {
                      if (curbookingData.val().status == 'ACCEPTED') {
                        firebase
                          ...
                          .update({
                            status: 'CANCELLED',
                            reason: this.state.radio_props[this.state.value]
                              .label
                          })
                          .then(() => {
                            firebase
                              .database()
                              .ref('/users/' + this.state.driverUID + '/')
                              .update({
                                queue: false
                              })
                              .then(() => {
                                updateUser(this.state.driverUID, {
                                  queue: false
                                })
                              })
                            ...
                            this.sendPushNotification(
                              ...
                              this.state.driverName +
                                ` ${languageJSON.has_cancelled_current_booking}`
                            )
                          })
                      }
                    } else {
          ...
          <View style={styles.alertModalInnerContainer}>
            <View style={styles.alertContainer}>
              <Text style={styles.rideCancelText}>
                {languageJSON.rider_cancel_text}
              </Text>
              <View style={styles.horizontalLLine} />
              <View style={styles.msgContainer}>
                <Text style={styles.cancelMsgText}>
                  {languageJSON.cancel_messege1} {this.state.currentBookingId}{' '}
                  {languageJSON.cancel_messege2}{' '}
                </Text>
              </View>
              <View style={styles.okButtonContainer}>
                <Button
                  ...
                  onPress={() => {
                    this.setState(
                      { alertModalVisible: false, currentBookingId: null },
                      () => {
  ...
  driverInfoModal() {
    return (
      <Modal
        animationType="none"
        transparent={true}
        visible={this.state.driverInfoModalVisible}
        onRequestClose={() => {
          this.setState({ driverInfoModalVisible: false })
        }}
      >
        <View style={styles.driverInfoModalContainer}>
          <View style={styles.driverInfoModalInnerContainer}>
            <View style={styles.driverInfoContainer}>
              <Text style={styles.driverAboutText}>{'Sobre o motorista'}</Text>
              <View style={styles.driverModalHorizontalLLine} />
              <View style={styles.driverModalMsgContainer}>
                <View style={styles.driverModalDetails}>
                  <View style={styles.driverPhotoContainerModal}>
                    <Image
                      source={
                        this.state.driverPic
                          ? { uri: this.state.driverPic }
                          : require('../../assets/images/profilePic.png')
                      }
                      style={styles.driverPhotoModal}
                    />
                  </View>
                  <View style={styles.driverModalNameContainer}>
                    <Text style={styles.driverNameText}>
                      {this.state.driverName ? this.state.driverName : null}
                    </Text>
                  </View>
                  <View style={styles.driverModalratingContainer}>
                    <StarRating
                      disabled={true}
                      maxStars={5}
                      starSize={height / 42}
                ...
                <View style={styles.driverModalDetails}>
                  <View style={styles.driverModalCabPhoto}>
                    {this.state.vehicleColor ? (
                      <Image
                        source={this.state.vehicleColor}
                        style={styles.cabImagePerson}
                      />
                    ) : (
                      <Image
                    ...
                    <View style={styles.driverModalCabNumber}>
                      <Text style={styles.driverModalCabNumberText}>
                        Tipo:{' '}
                        {this.state.driverName
                          ? this.state.driverName
                          : 'Não informado'}
                    ...
                    <View style={styles.driverModalCabNumber}>
                      <Text style={styles.driverModalCabNumberText}>
                        Placa:{' '}
                        {this.state.carNo ? this.state.carNo : 'Não informado'}
                      </Text>
                    </View>
                    <View style={styles.driverModalCabNumber}>
                      <Text style={styles.driverModalCabNumberText}>
                        Cor:{' '}
                        {this.state?.vehicleColorName
                          ? this.state?.vehicleColorName
                          : 'Não informado'}
                      </Text>
                    </View>
      ...
      <View style={styles.mainContainer}>
        ...
        <View style={styles.topContainer}>
          ...
          <View style={styles.topRightContainer}>
            <TouchableOpacity style={styles.whereButton}>
              <View style={styles.whereContainer}>
                <Text numberOfLines={1} style={styles.whereText}>
                  {this.state.region && this.state.region.whereText
            ...
            <TouchableOpacity style={styles.dropButton}>
              <View style={styles.whereContainer}>
                <Text numberOfLines={1} style={styles.whereText}>
                  {this.state.region && this.state.region.droptext
        ...
        <View style={styles.mapcontainer}>
          ...
          this.state.bookingStatus ? (
            <TrackNow
              duid={this.state.driverUID}
              alldata={this.state.region}
        ...
        <View style={styles.bottomContainer}>
          <View style={styles.otpContainer}>
            <Text style={styles.cabText}>
              {languageJSON.you_selected}{' '}
              <Text style={styles.cabBoldText}>{this.state.carType}</Text>
            </Text>
            <Text style={styles.otpText}>
              {this.state.otp ? languageJSON.otp + this.state.otp : null}
            </Text>
          </View>
          <View style={styles.cabDetailsContainer}>
            {this.state.bookingStatus === 'NEW' ? null : (
              <TouchableOpacity
                ...
                onPress={() => {
                  this.setState({ driverInfoModalVisible: true })
                }}
              >
                <View style={styles.cabName}>
                  <Text style={styles.cabNameText}>
                    {this.state.carModel ? this.state.carModel : null}
                  </Text>
                </View>
                ...
                <View style={styles.cabNumber}>
                  <Text style={styles.cabNumberText}>
                    {this.state.carNo ? this.state.carNo : null}
                  </Text>
                </View>
              </TouchableOpacity>
            )}
          ...
          <View style={styles.buttonsContainer}>
            ...
            <View style={styles.buttonContainer}>
              <Button
                disabled={this.state.bookingStatus === 'NEW'}
                ...
                titleStyle={styles.buttonTitleText}
                onPress={() => {
                  this.onPressCall('tel:' + this.state.driverContact)
                }}
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <Image
                  source={require('../../assets/images/load_ride.gif')}
                  resizeMode={'contain'}
                  style={{
                    width: 230,
                    height: 130,
                ...
                <View style={styles.buttonContainer}>
                  <Button
                    ...
                    titleStyle={styles.buttonTitleText}
                    onPress={() => {
                      this.onPressCancel('fromLoading')
                    }}
                    buttonStyle={styles.cancelButtonStyle}
...
const styles = StyleSheet.create({
  ...
  cabImagePerson: { width: 40, height: 20, marginBottom: 6 },
  ...
  // driver info modal
  driverInfoModalContainer: {
    flex: 1,
    justifyContent: 'center',
    backgroundColor: colors.GREY.background
  },
  driverInfoModalInnerContainer: {
    height: height * 0.85,
    width: width * 0.85,
    backgroundColor: colors.WHITE,
    alignItems: 'center',
  ...
  driverModalHorizontalLLine: {
    width: width - 110,
    height: 0.5,
    backgroundColor: colors.BLACK,
    alignSelf: 'center'
  },
  driverModalMsgContainer: {
    flex: 5.5,
    alignItems: 'center',
    justifyContent: 'center'
  },
  ...
  driverPhotoContainerModal: {
    marginTop: 15,
    justifyContent: 'flex-end',
    alignItems: 'center'
  },
  driverPhotoModal: { borderRadius: height / 5 / 2, width: 120, height: 120 },
  driverModalNameContainer: {
    marginTop: 15,
    alignItems: 'center',
    justifyContent: 'center'
  },
  driverModalratingContainer: {
    marginTop: 10,
    alignItems: 'center',
    justifyContent: 'center'
  },
